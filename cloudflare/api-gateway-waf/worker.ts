...
export default {
  async fetch(
    ...
    const skyfireHeaderExists =
      request.headers.has("kyapay-token") ||
      request.headers.has("skyfire-pay-id");
    ...
    if (skyfireHeaderExists) {
      ...
      try {
        ...
        if (!["kya+jwt", "kya-pay+jwt"].includes(`${protectedHeader.typ}`)) {
          ...
          return new Response(
            JSON.stringify({
              error:
                "Invalid typ - typ should be one of kya+jwt or kya-pay+jwt",
            }),
            {
              status: 401,
      ...
      if (!validator.isUUID(payload.jti)) {
        ...
        return new Response(
          JSON.stringify({
            error: "Invalid token ID (jti) - not a valid UUID",
          }),
          {
            status: 401,
            headers: { "content-type": "application/json" },
      ...
      if (!validator.isUUID(payload.aud)) {
        ...
        return new Response(
          JSON.stringify({
            error: "Invalid audience (aud) - not a valid UUID",
          }),
          {
            status: 401,
            headers: { "content-type": "application/json" },
